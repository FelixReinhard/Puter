...
public class InstructionsHelper {
    ...
    public static Instruction from(int word) {
        return from(new Integer[]{word}, 4);
    }
    ...
    public static Instruction from(Integer[] words, int amount) {
        ...
        return switch (bytes[0]) {
            ...
            case 0x08 -> new LaInstruction((byte) ((bytes[1] & 0b11110000) >>> 4),  (byte) (bytes[1] & 0b00001111), (short)ByteUtils.getNumberFromArray(bytes, 2, 4));
            ...
            case 0x71 -> new JmpInstruction((byte)((bytes[1] & 0xf0) >>> 4), ByteUtils.getNumberFromArray(bytes, 1, 4) & 0x0fffff);
            case 0x72 -> new JeqInstruction((byte)((bytes[1] & 0xf0) >>> 4), ByteUtils.getNumberFromArray(bytes, 1, 4) & 0x0fffff);
            case 0x73 -> new JneInstruction((byte)((bytes[1] & 0xf0) >>> 4), ByteUtils.getNumberFromArray(bytes, 1, 4) & 0x0fffff);
            case 0x74 -> new JlsInstruction((byte)((bytes[1] & 0xf0) >>> 4), ByteUtils.getNumberFromArray(bytes, 1, 4) & 0x0fffff);
            case 0x75 -> new JgeInstruction((byte)((bytes[1] & 0xf0) >>> 4), ByteUtils.getNumberFromArray(bytes, 1, 4) & 0x0fffff);

            case 0x10 -> new DbgInstruction((byte)((bytes[1] & 0xf0) >>> 4));

            case 0x11 -> new CmpInstruction((byte) ((bytes[1] & 0b11110000) >>> 4),  (byte) (bytes[1] & 0b00001111));
            case 0x12 -> new SyscallInstruction();
